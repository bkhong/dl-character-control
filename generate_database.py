...
def process_data(anim, phase, gait, type='flat'):
    ...
    """ Do FK """
    global_xforms = Animation.transforms_global(anim)
    global_positions = global_xforms[:,:,:3,3] / global_xforms[:,:,3:,3]  # (F, J, 3)
    global_rotations = Quaternions.from_transforms(global_xforms)  # (F, J) quats
    ...
    local_velocities = root_rotation[:-1] *  (global_positions[1:] - global_positions[:-1])  # (F-1, J, 3)
    local_rotations = ((root_rotation[:-1] * global_rotations[:-1]))  # (F-1, J) quats
